...
class _MountMapCanvasState extends State<MountMapCanvas> with SingleTickerProviderStateMixin {
        ...
        onTap: () {
          if (_multiConnectSourceId != null) {
            if (_multiConnectSourceId == node.id) {
              setState(() => _multiConnectSourceId = null);
            ...
            GridView.count(
              ...
              children: [
                ...
                _menuIconBtn(
                  ...
                  "Line",
                  node.parentId == null ? (isDark ? Colors.white10 : Colors.black12) : Colors.limeAccent,
                  () {
  ...
  void _showAlertEditor(NodeModel node, MountMapProvider provider) {
    ...
    DateTime selectedDate = node.alertDate ?? now.add(const Duration(hours: 1));
        ...
        builder: (context, setState) => AlertDialog(
              ...
              children: [
                ...
                Container(
                  ...
                  child: ListTile(
                    ...
                    onTap: () async {
                      final DateTime pickerNow = DateTime.now();
                      final DateTime today = DateTime(pickerNow.year, pickerNow.month, pickerNow.day);
                      final DateTime initialDate = selectedDate.isBefore(today) ? today : selectedDate;
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                final DateTime triggerTime = DateTime(
                  selectedDate.year,
                  selectedDate.month,
                  selectedDate.day,
                  selectedTime.hour,
                ...
                if (triggerTime.isBefore(DateTime.now().add(const Duration(seconds: 5)))) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text("Waktu pengingat harus lebih dari waktu sekarang."),
                ...
                provider.updatePeak(
                  node.id,
                  alertMessage: messageCtrl.text.trim().isEmpty ? null : messageCtrl.text.trim(),
                  alertDate: triggerTime,
                  alertEnabled: true,
                  clearAlertMessage: messageCtrl.text.trim().isEmpty,
                );
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            "Alert set for ${triggerTime.day}/${triggerTime.month} at ${selectedTime.hour}:${selectedTime.minute.toString().padLeft(2, '0')}",
                          ),
                        ),
                      ],
                    ),
            ...
            children: [
              Row(
                ...
                children: [
                  Row(
                    children: [
                      Icon(Icons.palette_rounded, color: isDark ? Colors.white : Colors.black, size: 22),
                      const SizedBox(width: 10),
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text("Ubah Nama Label", style: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontSize: 12)),
                const SizedBox(height: 10),
                TextField(
